...
@section('main')
    <main class="px-6">
        <h1 class="text-lg font-bold">Add a new Vault</h1>
        <form onsubmit="return confirm('Are you sure');" action="{{ route('vaults.store') }}" method="POST">
            @csrf
            <section class="mt-4">
                <div class="mb-4">
                    ...
                    <input id="name" type="text" class="border border-slate-200 rounded-md w-full"
                        value="{{ old('name') }}" placeholder="Vault" name="name" required>
                    @error('name')
                ...
                <div class="mb-4">
                    ...
                    <input id="address" type="text" class="border border-slate-200 rounded-md w-full"
                        value="{{ old('address') }}" placeholder="Jl. Merdeka" name="address" required>
                    @error('address')
                        <p class="mt-1 text-red-600">{{ $message }}</p>
                    @enderror
                </div>
